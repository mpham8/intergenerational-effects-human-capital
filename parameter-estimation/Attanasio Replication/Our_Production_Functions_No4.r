...
CES3_nocf <- function(alldata) {
  alldata$hc3 <- alldata$hc3 / max(alldata$hc3, na.rm=TRUE)
  alldata$govinvest3 <- alldata$govinvest3 / max(alldata$govinvest3, na.rm=TRUE)
  alldata$pinvest3 <- alldata$pinvest3 / max(alldata$pinvest3, na.rm=TRUE)
  out <- log(alldata$hc4)
  # Debug CES term
  s1 <- 0.3; s2 <- 0.2; rho <- -2
  ...
  cesout <- try(nlsLM(out ~ (1/rho)*log(s1 * hc3^rho + s2 * govinvest3^rho + (1-s1-s2) * pinvest3^rho) 
                      + delta + alpha2 * income3 + alpha3 * peducation, data = alldata,
                      start=c(delta=0.3, alpha2=0.05, alpha3=0.05, s1=0.3, s2=0.2, rho=0.5),
  ...
  if (length(summary(cesout)) == 11) {  
    ...
    names(estim) <- c("delta", "income3", "peducation", "hc3", "govinvest3", "pinvest3", "rho", "elast", "residual sd")
  } else {
...
CES3_cf <- function(alldata){
  ...
  inst3<-lm((log(pinvest3)) ~ (log(hc3)  +  log(centerprice3)+log(familyprice3)+log(peducation)+ log(income3)), data=alldata)
  cf         <- inst3$residuals
  ...
  cesout <- try(nlsLM(out ~ (1/rho)* log(s1 * hc3^rho + s2*govinvest3^rho + (1-s1-s2)* pinvest3^rho) 
                  +delta+ alpha*cf+ alpha2*income3 + alpha3*peducation, data = alldata,
                  start=c(delta=0.3, alpha=0, alpha2=.05, alpha3=.05, s1=0.3, s2=0.2, rho=0.5),
                  lower=c(-Inf, -Inf, -Inf, -Inf, 0, 0, -5),
                  upper=c(Inf, Inf, Inf, Inf, 1, 1, 5),
  ...
  if (length(summary(cesout))==11){  
    ...
    npar   <- nrow(summary(cesout)[[10]])
    elast  <-  1 / (1 - (summary(cesout)[[10]][npar,1]))
    s3     <-  1 - sum(summary(cesout)[[10]][5:6,1])
    residual_sd <- summary(cesout)[[3]]
    estim <- c(summary(cesout)[[10]][1:(npar-1),1], s3,summary(cesout)[[10]][npar,1],elast,residual_sd)
    names(estim) <- c("delta", "controlfn", "income3", "peducation",  "hc3", "govinvest3", "pinvest3", "rho", "elast","residual sd")
    
    gamma = summary(cesout)[[10]][2,1]
    structural_res = residuals(cesout) + gamma*cf
    reg     <- lm(structural_res  ~ cbind(log(hc3)  +  log(centerprice3)+log(familyprice3)+log(peducation)+ log(income3)), data=alldata)
...
CES2_nocf <- function(alldata){
  ...
  cesout <- try(nlsLM(out ~ (1/rho)* log(s1 * hc2^rho + s2*govinvest2^rho + (1-s1-s2)* pinvest2^rho) 
                  +delta+ alpha2*income2 + alpha3*peducation, data = alldata,
                  start=c(delta=0.3, alpha2=.05, alpha3=.05, s1=0.3, s2=0.2, rho=0.5),
                  lower=c(-Inf, -Inf, -Inf, 0, 0, -5),
                  upper=c(Inf, Inf, Inf, 1, 1, 5),
                  control=nls.lm.control(maxiter=1000)), silent=FALSE)
  ...
  if (length(summary(cesout))==11){  
    ...
    residual_sd <- summary(cesout)[[3]]
    estim <- c(summary(cesout)[[10]][1:(npar-1),1], s3,summary(cesout)[[10]][npar,1],elast,residual_sd)
    names(estim) <- c("delta", "income2", "peducation",  "hc2", "govinvest2", "pinvest2", "rho", "elast","residual sd")
...
CES2_cf <- function(alldata){
  ...
  inst2<-lm((log(pinvest2)) ~ (log(hc2)  +  log(centerprice2)+log(familyprice2)+log(peducation)+ log(income2)), data=alldata)
  cf         <- inst2$residuals
  ...
  cesout <- try(nlsLM(out ~ (1/rho)* log(s1 * hc2^rho + s2*govinvest2^rho + (1-s1-s2)* pinvest2^rho) 
                  +delta+ alpha*cf+ alpha2*income2 + alpha3*peducation, data = alldata,
                  start=c(delta=0.3, alpha=0, alpha2=.05, alpha3=.05, s1=0.3, s2=0.2, rho=0.5),
                  lower=c(-Inf, -Inf, -Inf, -Inf, 0, 0, -5),
                  upper=c(Inf, Inf, Inf, Inf, 1, 1, 5),
                  control=nls.lm.control(maxiter=1000)), silent=FALSE)
  ...
  if (length(summary(cesout))==11){  
    ...
    residual_sd <- summary(cesout)[[3]]
    estim <- c(summary(cesout)[[10]][1:(npar-1),1], s3,summary(cesout)[[10]][npar,1],elast,residual_sd)
    names(estim) <- c("delta", "controlfn", "income2", "peducation",  "hc2", "govinvest2", "pinvest2", "rho", "elast","residual sd")

    gamma = summary(cesout)[[10]][2,1]
    structural_res = residuals(cesout) + gamma*cf
    reg     <- lm(structural_res  ~ cbind(log(hc2)  +  log(centerprice2)+log(familyprice2)+log(peducation)+ log(income2)), data=alldata)
    coef   <-  as.vector(summary.lm(reg)[[4]][,1])
    se     <-  as.vector(summary.lm(reg)[[4]][,2])
    vcov   <-  as.matrix(vcov(reg))
  } else{
...
CES1_nocf <- function(alldata){
  ...
  cesout <- try(nlsLM(out ~ (1/rho)* log(s1 * hc1^rho + s2*govinvest1^rho + (1-s1-s2)* pinvest1^rho) 
                  +delta+ alpha2*income1 + alpha3*peducation, data = alldata,
                  start=c(delta=0.3, alpha2=.05, alpha3=.05, s1=0.3, s2=0.2, rho=0.5),
                  lower=c(-Inf, -Inf, -Inf, 0, 0, -5),
                  upper=c(Inf, Inf, Inf, 1, 1, 5),
                  control=nls.lm.control(maxiter=1000)), silent=FALSE)
  ...
  if (length(summary(cesout))==11){  
    ...
    residual_sd <- summary(cesout)[[3]]
    estim <- c(summary(cesout)[[10]][1:(npar-1),1], s3,summary(cesout)[[10]][npar,1],elast,residual_sd)
    names(estim) <- c("delta", "income1", "peducation",  "hc1", "govinvest1", "pinvest1", "rho", "elast","residual sd")
...
CES1_cf <- function(alldata){
  ...
  out<-log(alldata$hc2)
  #log(hc1) is taken out the regression because it has no variation (1 for everyone)
  inst1<-lm((log(pinvest1)) ~ (log(centerprice1)+log(familyprice1)+log(peducation)+ log(income1)), data=alldata)
  cf         <- inst1$residuals
  ...
  cesout <- try(nlsLM(out ~ (1/rho)* log(s1 * hc1^rho + s2*govinvest1^rho + (1-s1-s2)* pinvest1^rho) 
                  +delta+ alpha*cf+ alpha2*income1 + alpha3*peducation, data = alldata,
                  start=c(delta=0.3, alpha=0, alpha2=.05, alpha3=.05, s1=0.3, s2=0.2, rho=0.5),
                  lower=c(-Inf, -Inf, -Inf, -Inf, 0, 0, -5),
                  upper=c(Inf, Inf, Inf, Inf, 1, 1, 5),
                  control=nls.lm.control(maxiter=1000)), silent=FALSE)
  ...
  if (length(summary(cesout))==11){  
    ...
    residual_sd <- summary(cesout)[[3]]
    estim <- c(summary(cesout)[[10]][1:(npar-1),1], s3,summary(cesout)[[10]][npar,1],elast,residual_sd)
    names(estim) <- c("delta", "controlfn", "income1", "peducation",  "hc1", "govinvest1", "pinvest1", "rho", "elast","residual sd")

    gamma = summary(cesout)[[10]][2,1]
    structural_res = residuals(cesout) + gamma*cf
    reg     <- lm(structural_res  ~ cbind(log(centerprice1)+log(familyprice1)+log(peducation)+ log(income1)), data=alldata)
    coef   <-  as.vector(summary.lm(reg)[[4]][,1])
    se     <-  as.vector(summary.lm(reg)[[4]][,2])
    vcov   <-  as.matrix(vcov(reg))
  } else{
...
# CES period 3, with control fctns
...
# CES period 2, with control fctns
...
# CES period 1, with control fctns
...
rownames(trueCESperiod3)<-cbind("delta", "control function", "income 3", "highest grade completed mother",  "human capital 3", "government investment 3", "parental investment 3", "rho", "elasticity of substitution","residual sd")
colnames(trueCESperiod3)<-cbind("Human Capital without Control Function", "Human Capital with Control Function")
rownames(trueCESperiod2)<-cbind("delta", "control function", "income 2", "highest grade completed mother",  "human capital 2", "government investment 2", "parental investment 2", "rho", "elasticity of substitution","residual sd")
colnames(trueCESperiod2)<-cbind("Human Capital without Control Function", "Human Capital with Control Function")
rownames(trueCESperiod1)<-cbind("delta", "control function", "income 1", "highest grade completed mother",  "human capital 1", "government investment 1", "parental investment 1", "rho", "elasticity of substitution","residual sd")
colnames(trueCESperiod1)<-cbind("Human Capital without Control Function", "Human Capital with Control Function")
...
save(coef_ces1, file="coef_ces1.R")
  ...
  # Estimation on bootstrap data
    ...
    bootprod <-  function(b){  
      ...
      print(summary(alldatab[, c("hc3", "govinvest3", "pinvest3", "income3", "peducation")]))
      #attach(alldatab)
    ...
    bootCES3           <- array(0, dim=c(nrow(trueCESperiod3), 2, bsample))
    bootCES2           <- array(0, dim=c(nrow(trueCESperiod2), 2, bsample))
    bootCES1           <- array(0, dim=c(nrow(trueCESperiod1), 2, bsample))
    bootInvest3       <- array(0, dim=c(length(trueInvest3), 2, bsample))
    bootInvest2       <- array(0, dim=c(length(trueInvest2), 2, bsample))
    bootInvest1       <- array(0, dim=c(length(trueInvest1), 2, bsample))
    bootcoef_ces3    <- array(0, dim = c(length(coef_ces3), bsample))
    bootcoef_ces2    <- array(0, dim = c(length(coef_ces2), bsample))
    bootcoef_ces1    <- array(0, dim = c(length(coef_ces1), bsample))
    
    npar <- nrow(trueCESperiod3)      
    npar1 <- npar - 1
    ...
    for (b in 1:bsample) {
      ...
      bootInvest3[,,b] <- matrix(estim[[7]], nrow=nrow(bootInvest3), ncol=1)
      bootInvest2[,,b] <- matrix(estim[[8]], nrow=nrow(bootInvest2), ncol=1)
      bootInvest1[,,b] <- matrix(estim[[9]], nrow=nrow(bootInvest1), ncol=1)
...
for (i in 1:npar){ 
  ...
  for (l in 1:(2)){
    ...
    cestable_3[(i*3), l] <- paste("[", round(quantile(bootCES3[i,l,], 0.05, na.rm=TRUE), 3), ",",       # Confidence interval 
...
rownames(cestable_3)<- cbind("delta","","",
                              "control function", "","",
                              "income 3","","",
...
investtable_3       <- matrix(0, (length(trueInvest3)*3), 1)


for (i in 1:length(trueInvest3)) {
...
rownames(investtable_3)<- cbind("Intercept","","",
                                 "Human Capital 3","","",
                                 "Center Price 3","","",
                                  "Family Price 3","","",
                                  "Highest Grade Completed Mother","","",
...
for (i in 1:npar) { 
  ...
  for (l in 1:(2)) {
    ...
    cestable_2[(i*3), l] <- paste("[", round(quantile(bootCES2[i,l,], 0.05, na.rm=TRUE), 3), ",",       # Confidence interval 
...
rownames(cestable_2) <- cbind("delta","","",
                              "control function", "","",
                              "income 2","","",
                              "highest grade completed mother","","",
                              "human capital 2","","",
...
investtable_2 <- matrix(0, (length(trueInvest2)*3), 1)

for (i in 1:length(trueInvest2)) {
...
rownames(investtable_2) <- cbind("Intercept","","",
                                 "Human Capital 2","","",
                                 "Center Price 2","","",
                                 "Family Price 2","","",
                                 "Highest Grade Completed Mother","","",
...
for (i in 1:npar) { 
  ...
  for (l in 1:(2)) {
    ...
    cestable_1[(i*3), l] <- paste("[", round(quantile(bootCES1[i,l,], 0.05, na.rm=TRUE), 3), ",",       # Confidence interval 
...
rownames(cestable_1) <- cbind("delta","","",
                              "control function", "","",
                              "income 1","","",
                              "highest grade completed mother","","",
                              "human capital 1","","",
...
investtable_1 <- matrix(0, (length(trueInvest1)*3), 1)

for (i in 1:length(trueInvest1)) {
...
rownames(investtable_1) <- cbind("Intercept","","",
                                 "Center Price 1","","",
                                 "Family Price 1","","",
                                 "Highest Grade Completed Mother","","",
                                 "Income 1","","")
