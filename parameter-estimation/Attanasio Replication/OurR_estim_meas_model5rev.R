...
###############################################################
#####Added min iteration here
#######################
min_iter <- 10
while ((iter < min_iter) || (abs((loglik[iter+1] - loglik[iter]) / loglik[iter]) > conv)) {
iter      <- iter + 1
estep     <- estepfn(mstep,y)
mstep     <- mstepfn(estep,y)
loglik    <- rbind(loglik, estep[[2]])
...
plot(loglik, type = 'l', main = "EM Log-Likelihood by Iteration", xlab = "Iteration", ylab = "Log-Likelihood")
