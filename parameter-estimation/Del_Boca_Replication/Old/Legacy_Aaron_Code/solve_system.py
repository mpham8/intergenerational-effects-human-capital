...
################## Aaron's Test to Compare Python and Julia Implementations ##################

NUM_PERIODS = 4

def create_simple_household():
    """
    Create a single household array with shape (NUM_PERIODS, 7) for testing.
    Uses constant values for wage rates and government inputs.
    """
    household = np.zeros((NUM_PERIODS, 7))
    
    # Parent human capital (constant across periods)
    h_val = 0.7  # Same as original run_simulation
    household[:, 0] = h_val
    
    # Wage rates (constant)
    household[:, 1] = 1.0  # Constant wage rate, as in create_households initial value
    
    # Government inputs (constant, using approximate mean from original)
    household[:, 2] = 0.013  # Approximate mean of government_input_means
    
    return household

def run_test_simulation(rho_val, rho_e_val):
    """
    Run a single simulation for given rho and rho_e values.
    Returns a dictionary with results for saving to CSV.
    """
    try:
        # Construct parameters: [delta, rho, rho_e, theta1, theta2, theta3]
        parameters = [20.0, rho_val, rho_e_val, 0.3, 0.2, 0.2]  # Defaults from parameters_to_optimize
        ...
        # Extract results: solution is (4, 4) array [c_t, e_t, l_t, h_{t+1}]
        c1, e1, l1, h2 = solution[0, :]
        c2, e2, l2, h3 = solution[1, :]
        c3, e3, l3, h4 = solution[2, :]
        c4, e4, l4, h5 = solution[3, :]
        
        # predh5final is from period 1 output
        predh5final = solution[0, 3]  # h5 from solve_bounded_system_t1
...
def run_all_test_simulations():
    """
    Run simulations over a small parameter grid and save results to CSV.
    """
    # Small parameter grid for testing (within valid range -5 to 0.5)
    rho_values = np.linspace(-2, 0.5, 5)  # [-2, -1.125, -0.25, 0.625, 0.5]
    ...
    for rho_val, rho_e_val in product(rho_values, rho_e_values):
        ...
        # Save intermediate results every 5 simulations
        if completed % 5 == 0:
            df = pd.DataFrame(results)
            df.to_csv('test_results_intermediate.csv', index=False)
            print(f"Saved intermediate results ({completed}/{total_combinations})")
    
    # Save final results
    df = pd.DataFrame(results)
    df.to_csv('test_results.csv', index=False)
    print("All test simulations completed. Results saved to test_results.csv")
