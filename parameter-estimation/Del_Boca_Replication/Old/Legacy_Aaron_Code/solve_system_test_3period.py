...
def test_equivalence(num_trials: int = 5, tol: float = 1e-10) -> None:
    """Test solve_household with fixed inputs for comparison with Julia."""
    import numpy as np
    # Fixed inputs for consistency
    trials = [
        {
            'params': [0.0, -0.1254598811526375, -0.24292851141124316, 0.7319939418114051, 0.5986584841970366, 0.15601864044243652],
            'hh': np.array([
                [0.65599452, 1.040446, 0.0293982],
                [0.55808361, 1.08322903, 0.02664885],
                [1.36617615, 0.8082338, 0.01424678]
            ])
        },
        # Add more trials if needed, using Python Trial 2-5 params and hh
    ]
    for trial_idx, trial in enumerate(trials[:num_trials]):
        params = trial['params']
        hh = trial['hh']
        result = solve_household(params, hh)
        print(f"Trial {trial_idx + 1} - Params: {params}")
        print(f"Household:\n{hh}")
        print(f"Result:\n{result}\n")
