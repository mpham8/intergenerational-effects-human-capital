...
# Import the shared SMM orchestration; it loads Rev3 and threaded DX-NES.
if !isdefined(@__MODULE__, :two_step_smm)
    include(joinpath(@__DIR__, "Julia_DelBoca_SMM.jl"))
end

# Constants

# Model dimensions come from Rev3 so solver and estimator cannot drift apart.
const NUM_PARAMETERS = length(PARAMETER_NAMES)
...
# MCMC
const INITIAL_PARAMS = [10.0, -0.3, 0.4, 0.15, 0.1]
...
const PROPOSAL_DISTS = Dict(
    ...
    "perceived_theta_h_1" => Dict(
        "type" => "truncated_normal", 
        "scale" => 0.05,
        "range" => (0.05, 0.95)
    ), 
    "perceived_theta_h_2" => Dict(
        "type" => "truncated_normal", 
        "scale" => 0.05,
        "range" => (0.05, 0.95)
    ), 
    "perceived_theta_h_3" => Dict(
        "type" => "truncated_normal", 
        "scale" => 0.05,
        "range" => (0.05, 0.95)
    )
...
function SMMAdaptiveMetropolis(;
    ...
    households::Union{Array{Float64, 3}, Nothing} = nothing,
    ...
    param_names = something(parameter_names, copy(PARAMETER_NAMES))

    param_bounds = something(
        parameter_bounds,
        collect(zip(DXNES_LOWER_BOUNDS, DXNES_UPPER_BOUNDS)),
    )

    init_params = something(initial_params, copy(parameters))
    ...
    simulated_households = households === nothing ?
                           create_households(initial_households) : copy(households)
    num_current_households = size(simulated_households, 1)
...
function generate_placeholder_empirical_moments(initial_households::Int, rng::AbstractRNG)
    ...
    target_parameters = [20.0, -0.2, 0.3, 0.2, 0.2]
    ...
    for _ in 1:2 # Increase the number of loops for greater accuracy of "empirical" parameters
        ...
        solved_households = solve_households(copy(households_empirical), target_parameters)
...
function check_parameter_constraints(estimator::SMMAdaptiveMetropolis, params::Vector{Float64})
    """Check whether parameters satisfy the Rev3 bounds."""
    ...
    # The perceived period-specific theta_h values are separate CES weights,
    # not components of one simplex, so they do not need to sum to one.
    return true
end

function simulate_moments(estimator::SMMAdaptiveMetropolis, params::Vector{Float64})
    """Simulate moments using the estimator's fixed common random numbers."""
    solved_households = solve_households(copy(estimator.simulated_households), params, 0)
    return moments(solved_households)
end

function compute_smm_objective(estimator::SMMAdaptiveMetropolis, params::Vector{Float64})
    """Compute the same standardized, weighted objective used by DX-NES."""
    return rev3_smm_objective(
        params,
        estimator.empirical_moments,
        estimator.simulated_households;
...
function compute_diagnostics(estimator::SMMAdaptiveMetropolis)
    ...
    # Effective sample size from positive autocorrelations. Only the lags used
    # by the estimate are computed, avoiding the old O(n^2) full convolution.
    ess = Vector{Float64}()
    for i in 1:size(chain_array, 2)
        param_chain = chain_array[:, i]
        param_centered = param_chain .- mean(param_chain)
        variance_sum = dot(param_centered, param_centered)
        if variance_sum <= eps(Float64)
            push!(ess, 1.0)
            continue
        end

        tau = 1.0
        for lag in 1:min(length(param_chain) ÷ 4, length(param_chain) - 1)
            rho = dot(
                @view(param_centered[1:end-lag]),
                @view(param_centered[1+lag:end]),
            ) / variance_sum
            rho <= 0 && break
            tau += 2 * rho
        end

        push!(ess, length(param_chain) / tau)
    end
...
function compute_optimal_weighting_matrix!(
    estimator::SMMAdaptiveMetropolis;
    parameter_values::Vector{Float64}=estimator.initial_params,
    draws::Int=SMM_WEIGHTING_DRAWS,
    ridge::Float64=SMM_WEIGHTING_RIDGE,
)
    """Estimate stage-two weights from household-bootstrap moment variation."""
    ...
    estimator.weighting_matrix = compute_weighting_matrix(moment_draws; ridge=ridge)
    println("Computed bootstrap SMM weighting matrix from $draws household draws")
    return estimator.weighting_matrix
end
...
function run_mcmc!(estimator::SMMAdaptiveMetropolis; 
    ...
    return Dict(
        ...
        "smm_values" => copy(estimator.smm_values),
        "diagnostics" => diagnostics,
        "total_time" => total_time
    )
end

function reset_mcmc_state!(estimator::SMMAdaptiveMetropolis)
    empty!(estimator.chain)
    empty!(estimator.accepted_chain)
    empty!(estimator.smm_values)
    empty!(estimator.acceptance_history)
    empty!(estimator.household_count_history)
    empty!(estimator.parameter_std_history)
    empty!(estimator.computation_times)
    estimator.current_iteration = 0
    estimator.n_accepted = 0
    estimator.increase_household_spacer = 0
    estimator.covariance_matrix = nothing
    estimator.mean_params = nothing
    return estimator
end
...
function run_two_step_smm!(estimator::SMMAdaptiveMetropolis;
                          ...
                          stage1_dxnes_evaluations::Int = DXNES_MAX_EVALUATIONS,
                          stage2_dxnes_evaluations::Int = DXNES_MAX_EVALUATIONS,
                          dxnes_population_size::Int = DXNES_POPULATION_SIZE,
                          weighting_draws::Int = SMM_WEIGHTING_DRAWS,
                          weighting_ridge::Float64 = SMM_WEIGHTING_RIDGE,
    ...
    Threaded DX-NES finds the identity-weighted first-stage estimate, household
    bootstrap draws estimate the second-stage weights, and DX-NES re-optimizes
    under those weights before adaptive Metropolis samples the final stage.
    """
    
    println("="^60)
    println("STARTING TWO-STEP SMM PROCEDURE")
    println("="^60)
    stage1_iterations >= 0 || error("stage1_iterations cannot be negative.")
    stage2_iterations > 0 || error("stage2_iterations must be positive.")
    weighting_draws >= 2 || error("At least two weighting draws are required.")
    
    lower_bounds = first.(estimator.parameter_bounds)
    upper_bounds = last.(estimator.parameter_bounds)

    # Stage 1: identity weighting and a global mode search.
    println("\nSTAGE 1: Finding the identity-weighted mode with threaded DX-NES...")
    estimator.is_second_stage = false
    estimator.weighting_matrix = Matrix{Float64}(I, NUM_MOMENTS, NUM_MOMENTS)
    stage1_optimizer = optimize_parameters_dxnes(
        estimator.empirical_moments,
        estimator.simulated_households;
        lower_bounds=lower_bounds,
        upper_bounds=upper_bounds,
        max_evaluations=stage1_dxnes_evaluations,
        population_size=dxnes_population_size,
        random_seed=estimator.random_seed,
        optimizer_threads=optimizer_threads,
        weighting_matrix=estimator.weighting_matrix,
        standardize_moments=true,
        initial_guess=clamp.(estimator.initial_params, lower_bounds, upper_bounds),
    )
    estimator.initial_params = stage1_optimizer.parameters
    reset_mcmc_state!(estimator)

    # A stage-one chain is optional because the weighting matrix now comes from
    # household bootstrap draws. Skipping it by default saves many large-N solves.
    stage1_results = if stage1_iterations > 0
        println("STAGE 1: Sampling optional identity-weighted diagnostics...")
        run_mcmc!(estimator,
    ...
    compute_optimal_weighting_matrix!(
        estimator;
        parameter_values=stage1_optimizer.parameters,
        draws=weighting_draws,
        ridge=weighting_ridge,
    )
    
    # Stage 2: re-optimize under the new weights before sampling.
    println("\nSTAGE 2: Finding the optimally weighted mode with threaded DX-NES...")
    estimator.is_second_stage = true
    stage2_seed = estimator.random_seed === nothing ? nothing : estimator.random_seed + 1
    stage2_optimizer = optimize_parameters_dxnes(
        estimator.empirical_moments,
        estimator.simulated_households;
        lower_bounds=lower_bounds,
        upper_bounds=upper_bounds,
        max_evaluations=stage2_dxnes_evaluations,
        population_size=dxnes_population_size,
        random_seed=stage2_seed,
        optimizer_threads=optimizer_threads,
        weighting_matrix=estimator.weighting_matrix,
        standardize_moments=true,
        initial_guess=stage1_optimizer.parameters,
    )
    estimator.initial_params = stage2_optimizer.parameters
    reset_mcmc_state!(estimator)
    estimator.is_second_stage = true

    println("STAGE 2: Sampling from a clean chain at the optimally weighted mode...")
